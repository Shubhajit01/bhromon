import { USER_TIME_ZONE_COOKIE } from './user-time-zone';
...
export function needsTimeZoneCookie(request: Request) {
  ...
  const hasTimeZoneCookie = request.headers
    .get('cookie')
    ?.split(';')
    .some((cookie) => cookie.trim().startsWith(`${USER_TIME_ZONE_COOKIE}=`));
...
export function getTimeZoneBootstrapResponse() {
  return new Response(
    `<!doctype html><script>
const name=${JSON.stringify(USER_TIME_ZONE_COOKIE)};
...
export function getTimeZoneFallbackResponse(request: Request) {
  ...
  return new Response(
    `<!doctype html><script>
const saved=document.cookie.split('; ').some(cookie=>cookie===${JSON.stringify(`${USER_TIME_ZONE_COOKIE}=${encodedTimeZone}`)});
if(saved){location.replace(${JSON.stringify(url.toString())})}else{document.write('Bhromon could not save your time zone. Please enable cookies and reload.')}
</script>`,
    {
      headers: {
        'Cache-Control': 'private, no-store',
        'Content-Type': 'text/html; charset=utf-8',
        'Set-Cookie': `${USER_TIME_ZONE_COOKIE}=${encodedTimeZone}; Max-Age=31536000; Path=/; SameSite=Lax${secure}`,
      },
    },
  );
}
